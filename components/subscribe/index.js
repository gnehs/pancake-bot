			...
			if (args[0] !== '') {
				for (id in subscribeIdList) {
					if (args[0] == id) {
						...
						console.log(`[Subscribe] ${ctx.from.username}(${ctx.from.id}) subscribe ${id}`, `${args[1] || ''} in ${ctx.chat.id}(${ctx.chat.title || ctx.from.first_name})`)
					}
				}
			} else {
			...
			if (args[0] !== '') {
				for (id in subscribeIdList) {
					if (args[0] == id) {
						...
						console.log(`[Unsubscribe] ${ctx.from.username}(${ctx.from.id}) unsubscribe ${id}`, `${args[1] || ''} in ${ctx.chat.id}(${ctx.chat.title || ctx.from.first_name})`)
					}
				}
			} else {
				ctx.reply(`❌ 請在後面加上訂閱編號`, { reply_to_message_id: ctx.message.message_id })
