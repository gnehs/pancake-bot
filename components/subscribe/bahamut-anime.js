...
async function sendData() {
  ...
  for (item of fetchedData.newEpisode)
    if (!sentEpisode[item.id]) newEpisode.push(item);
  for (item of fetchedData.recentAdded)
    if (!sentEpisode[item.id]) recentAdded.push(item);
  // 重置已發送ㄉ資料
  sentEpisode = {};
  for ({ id } of fetchedData.newEpisode) sentEpisode[id] = true;
  for ({ id } of fetchedData.recentAdded) sentEpisode[id] = true;
